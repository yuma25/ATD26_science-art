...
### 2026-05-06 (コンセプト移行と機能強化)
- **コンセプト移行**: 「生物標本」から「絵画コレクション」へ名称とマスターデータを刷新。
- **詳細ビューワーの2D画像化**: Meshopt読み込みエラーを完全に解決するため、3D表示を停止し豪華な額縁付きの2D画像表示に刷新。
- **AR表示の最適化**: スキャン時の引き続き高品質な3Dモデル（蝶、クジラ等）を表示するハイブリッド仕様を実現。
- **SWRの導入**: ホーム画面のデータ取得にSWRを導入。キャッシュ活用により画面遷移時の「一瞬白くなる」現象を解消し、体感速度を劇的に向上。
- **景品交換システム**: コンプリート時に「交換チケット（横長UI）」を発行し、2段階確認を経て景品と交換（DB永続化）できる機能を実装。
- **外部キー制約の自動修復**: プロフィール未作成状態でバッジ獲得が走った際のエラーを自動的に自己修復するロジックをBadgeServiceに実装。

### 2026-05-07
- **ユーザーID発行タイミングの最適化**:
  - サイトアクセス時の自動的な匿名サインインを廃止。
  - 「来場人数入力」が確定した瞬間に初めて ID を発行し、データベースに登録するフローに変更。
  - これにより、管理者ボタンのクリックや誤アクセスで不要な ID が発行される問題を解消。
- **Makefile の内容削除**:
  - プロジェクトの簡素化のため、使用しなくなった `Makefile` の内容を空にしました。
- **来場者数入力モーダルの表示不具合修正**:

  - `frontend/hooks/useHome.ts`: マウント時に `signInAnonymously()` を自動実行するように修正。これにより初回訪問時でも `userId` が確保され、モーダルが正しく表示されるようになった。


- **AR 3Dモデルのサイズと位置の調整**: 全体的にモデルが小さいとの要望を受け、backend/lib/specimens/ 配下の全設定ファイルの scale を約1.5倍〜2倍に拡大。また、position や innerAnimation を調整してカメラ（Z軸正方向）に近づけることで、迫力のある表示に改善した。

- **AR 3Dモデルの再調整（調和重視）**: 迫力重視の拡大方針から、ARマーカーである絵画との調和を優先する方針に変更。全モデルの scale を縮小し、カメラに近づけすぎない適切な位置（Z軸の調整）に再設定した。

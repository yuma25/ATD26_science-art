...
├── scripts/            # 開発・運用支援用スクリプト
├── Makefile            # プロジェクトセットアップ用の自動化コマンド
...
本プロジェクトでは `Makefile` を使用して、開発環境の初期構築を簡略化できます。

### 1. プロジェクトの初期化

依存関係のインストールと、AI 駆動開発に必要なログファイルの生成を一度に行います。

```bash
make setup
```

※ 手動で行う場合は `pnpm install` を実行してください。
